/**
* @copyright 2025 - Max Bebök
* @license MIT
*/
#include "../main.h"
#include "../renderer/rdp.h"
#include "../renderer/dpl.h"
#include "../framework/testGroup.h"
#include "../framework/testSurface.h"

namespace
{
  constexpr uint64_t OTHER_MODE = RDP::setOtherModes(RDP::OtherMode()
    .cycleType(RDP::CYCLE::FILL)
    .ditherRGBA(RDP::DitherRGB::DISABLED)
    .ditherAlpha(RDP::DitherAlpha::DISABLED)
    .setAA(false)
    .forceBlend(false)
    .setImageRead(false)
    .setDepthWrite(false)
  );
}

namespace Tests::RDPRectNoSyncFill
{
  TestGroup create()
  {
    TestGroup group{"RDP Rect No-Sync-Fill"};

    for(int i=0; i<20; ++i)
    {
      group.test("Rect Env " + std::to_string(i), [i](Assert& assert)
      {
        TestSurface surf{FMT_RGBA32, 320-32, 180};
        surf.attachAndClear();

        RDP::DPL dplTri{4000};
        dplTri.add(RDP::setOtherModes(OTHER_MODE));

        float baseSizeY = (i*6)+1;
        float baseSizeX = 1;
        if(i >= 10)
        {
          baseSizeX = ((i-10)*10)+1;
          baseSizeY = 8;
        }

        float pos[2]{0.0f, 2};
        for(int y=0;  y<16; ++y)
        {
          pos[0] = 2;
          for(int x=0;  x<32; ++x)
          {
            dplTri // do multiple syncs here, otherwise the very first rects flicker on hardware
              .add(RDP::syncPipe()).add(RDP::syncPipe()).add(RDP::syncPipe())
              .add(RDP::setFillColor32({0xFF, 0xFF, 0xFF, 0xFF}))
              .add(RDP::fillRectSize(
                pos[0], pos[1], x+baseSizeX-1, y+baseSizeY-1
              ))
              .add(RDP::setFillColor32({0xFF, 0x00, 0x00, 0xFF}))
              .add(RDP::setFillColor32({0x00, 0xFF, 0x00, 0xFF}))
              .add(RDP::setFillColor32({0x00, 0x00, 0xFF, 0xFF}))
            ;
            pos[0] += x+baseSizeX+1;

            if(pos[0] + x + baseSizeX + 1 > surf.getWidth()) {
              break;
            }
          }
          pos[1] += y+baseSizeY+1;
          if((pos[1]+y+baseSizeY+1) > surf.getHeight()) {
            break;
          }
        }

        dplTri.runSync();

        assert.surface(surf);
        surf.draw(16, 48);
      });
    }

    return group;
  }
}
